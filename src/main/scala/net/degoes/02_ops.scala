...
object input_stream {
   ...
   */
  lazy val solution: IStream = primary.orElse(secondaries.foldLeft(IStream.empty)(_ ++ _))
...
object email_filter {
   ...
   */
  lazy val emailFilter1 = subjectContainsDiscount && bodyContainsN95 && !addressToJohnDoe

  def subjectContainsDiscount = EmailFilter.subjectContains("discount")
  def bodyContainsN95 = EmailFilter.bodyContains("N95")
  def addressToJohnDoe = EmailFilter.recipientIs(Address("john@doe.com"))
}
...
object contact_processing {
  ...
  final case class SchemaMapping(map: ContactsCSV => MappingResult[ContactsCSV]) { self =>
     ...
     */
    def +(that: SchemaMapping): SchemaMapping = SchemaMapping { csv => self.map(csv).flatMap(that.map) }
    ...
    def protect(columnNames: Set[String]): SchemaMapping =
      SchemaMapping { csv => self.map(csv).map(_.copy(schema = SchemaCSV(columnNames.toList))) }
  }
  ...
  object SchemaMapping {
    ...
    def combine(leftColumn: String, rightColumn: String)(newName: String)(
      ...
      SchemaMapping { csv => MappingResult.fromOption(csv.combine(leftColumn, rightColumn)(newName)(f), "Invalid column name")}
    ...
    def relocate(column: String, j: Int): SchemaMapping = SchemaMapping { csv =>
      MappingResult.fromOption(csv.relocate(column, j), "Invalid column")
    }
    ...
    def delete(name: String): SchemaMapping = SchemaMapping {
      csv => MappingResult.Success(Nil, csv.delete(name))
    }
  }
   ...
   */
  lazy val schemaMapping: SchemaMapping = SchemaMapping.rename("email", "email_address") +
    ...
    SchemaMapping.relocate("email_address", 1)
...
object education {
  ...
  final case class Quiz(run: () => QuizResult) { self =>
    ...
    def check(f: QuizResult => Boolean)(ifPass: Quiz, ifFail: Quiz): Quiz =
      Quiz{() => {
        val r = self.run()
        val result: QuizResult = if (f(r)) ifPass.run() else ifFail.run()
        r + result
      }}
  }
  object Quiz {
  ...
  lazy val exampleQuiz: Quiz =
    Quiz(Question.TrueFalse("Is Scala the best programming language on planet earth?", Checker.isTrue(10))) +
      Quiz(Question.MultipleChoice("What is the capital of Great Britain?", Vector("London", "Bristol", "Cambridge"),
        Checker.isMultipleChoice(15)(1))) +
      Quiz(Question.TrueFalse("Bonus question", Checker.isTrue(20))).bonus
        .check(_.bonusPoints > 0)(Quiz.empty, Quiz(Question.TrueFalse("Simple bonus question", Checker.isFalse(10))).bonus)
}
